...
Page({
  data: {
    ...
    outcome: '',
    outcomeIndex: -1,
    outcomeOptions: ['治愈', '好转', '未愈', '死亡', '自动出院', '转院', '住院', '自主离院', '其他'],
  ...
  handleOcrTextEdited: function(data) {
    ...
    if (idx < 0 || !this.data.ocrDataList || !this.data.ocrDataList[idx]) return;
    ...
    // 如果是编辑已有患者模式，也更新患者数据
    if (this.data.editMode && this.data.editPatientId) {
      app.updatePatient(this.data.editPatientId, {
        ocrResults: list,
        ocrResult: item
      }).catch((err) => {
  ...
  showOcrResultModal: function(text) {
    ...
    // 显示完整内容（最多5000字符）
    var displayText = text.length > 5000 ? text.substring(0, 5000) + '...[内容过长已截断]' : text;
    wx.showModal({
      title: 'OCR识别结果',
      content: displayText,
      confirmText: '复制全文',
  ...
  // 编辑指定索引的OCR内容（跳转到文本编辑页面）
  editOcrItem: function(e) {
    var idx = e.currentTarget.dataset.idx;
    var item = this.data.ocrDataList[idx];
    if (!item) return;
    var originalText = item.formattedText || item.rawText || '';
    ...
    wx.navigateTo({
      url: '/pages/textedit/textedit?editIdx=' + idx
    });
  },
