...
class UserManager {
    ...
    try {
      ...
      if (legacyData && Array.isArray(legacyData) && !migrationFlag) {
        ...
        if (users.length === 0) {
          ...
          console.log('默认密码:', randomPassword);
      ...
      for (var i = 0; i < doctors.length; i++) {
        ...
        if (existingUser) {
          ...
          if (existingUser.password !== correctPassword) {
            console.log('[presetDoctors] 密码不一致，修正:', docName, '| 存储:', existingUser.password, '| 正确:', correctPassword);
          ...
          // 账户缺失，创建新账户
          console.log('[presetDoctors] 账户不存在，创建:', docName, '| 密码:', correctPassword);
          var newUser = {
    ...
    try {
      ...
      if (admin) {
        if (admin.password !== correctPassword) {
          console.log('[ensureAdmin] 管理员密码不一致，正在修正:', admin.password, '->', correctPassword);
  ...
  ensureTestAccount() {
    try {
      ...
      console.log('测试账号已创建 (test/123456)');
    } catch (e) {
